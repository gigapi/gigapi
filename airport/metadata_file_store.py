...
from airport.model import encode_custom, decode_custom, MetaStore, TableFile, MergePlansByFolder
...
""", [schema_blob])
    ...
    def load_merge_planner(self, files: list[TableFile]):
        merge_plans = self.conn.query("SELECT plans FROM merge_plans WHERE id = 1").fetchone()
        if merge_plans:
    ...
    def load_delete_planner(self):
        delete_plans = self.conn.query("SELECT plans FROM merge_plans WHERE id = 2").fetchone()
        if delete_plans:
    ...
    def on_merge_planner_change(self, planner: MergePlanner):
        ...
        self.conn.execute("""
INSERT INTO merge_plans (id, plans)
VALUES (1, ?)
